        ...
        <el-table :data="rolelist" border stripe>
            <!-- 展开列 -->
                 ...
                 <template slot-scope='scope'>
                   <el-row :class='["bdbottom","vcenter",i1===0 ? "bdtop" : ""]' v-for="(item,i1) in scope.row.children" :key="item.id">
                        ...
                        <!-- 渲染二级和三级权限 -->
                         <el-col :span="19">
                          ...
                          <el-row :class="[i2===0 ? '' :'bdtop','vcenter']" v-for="(item2,i2) in item.children" :key="item2.id">
                           ...
                           <!-- 三级 -->
                           <el-col :span="18">
                          <el-tag :class='i3===0 ? "" : "bdtop"' v-for="(item3,i3) in item2.children" :key="item3.id"  type="warning" @close='removeRightById(scope.row,item3.id)' closable>
                          {{item3.authName}}
                         </el-tag>
                           </el-col>
                          </el-row>
          ...
          <el-table-column label="操作" width="300px">
              <template slot-scope="scope">
                  ...
                  <el-button size="mini" type="danger" icon="el-icon-delete" @click="showDeleteRoleDialog(scope.row.id)">删除</el-button>
...
<!-- 编辑角色弹框 -->
    <el-dialog
      ...
      <span>
        <el-form :model="editRoleForm" :rules="roleRules" ref="editRoleRef" label-width="100px" >
...
export default {
  ...
  methods: {
    ...
    async deleteRoleInfo(){
      const {data: res} = await this.$http.delete('roles/'+this.deleteRoleForm.roleId)
